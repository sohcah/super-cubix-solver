...
export function solveForCube(cube: Cube) {
    const expectedRotationlessShapeId = new Cube().rotationlessShapeId;

    const queue: [Cube, string[]][] = [[cube, []]];
    ...
    while (queue.length) {
        ...
        for (let topIndex = currentCube.top.length; topIndex > 0; topIndex--) {
            ...
            for (let bottomIndex = currentCube.bottom.length; bottomIndex > 0; bottomIndex--) {
                ...
                if (newCube) {
                    ...
                    queue.push([newCube, [...moves, `${topRotations},${bottomRotations}`]]);
                }
            }
        }
    }
...
export function solveForColours(cube: Cube) {
    const expectedRotationlessId = new Cube().rotationlessId;
